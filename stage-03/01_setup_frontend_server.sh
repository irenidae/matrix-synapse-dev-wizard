...
cat <<CONF
------------------------------------------------
Copy [Stage 3] Tokens to Crypto Notes/KeePassXC
------------------------------------------------
[Stage 3] Deploy Token:
${deploy_key}

[Stage 3] Support Token:
${support_key}
------------------------------------------------
CONF
}
...
configure_apparmor() {
    ...
    cat <<'CONF' | indent -4 | install -D -m 0644 -o root -g root /dev/stdin /etc/apparmor.d/docker-frontend-nginx
    #include <tunables/global>
    profile docker-frontend-nginx flags=(attach_disconnected,mediate_deleted) {
      signal,
...
CONF
    cat <<'CONF' | indent -4 | install -D -m 0644 -o root -g root /dev/stdin /etc/apparmor.d/docker-frontend-tunnel
    #include <tunables/global>
...
CONF
    ...
    apparmor_parser -r /etc/apparmor.d/docker-frontend-nginx || true
    apparmor_parser -r /etc/apparmor.d/docker-frontend-tunnel || true
}
configure_seccomp() {
    install -d -m 0755 -o root -g root /etc/docker/
    cat <<'CONF' | indent -4 | install -D -m 0644 -o root -g root /dev/stdin /etc/docker/seccomp-frontend.json
    {
...
setup_frontend_updater_service() {
    cat <<'CONF' | indent -4 | install -D -m 0755 -o root -g root /dev/stdin /usr/local/sbin/frontend-updater
    #!/bin/bash
    set -Eeuo pipefail
    IFS=$'\n\t'
    ...
    prod_net_external="frontend_external"
    ...
    cleanup_rollback_tags() {
        docker images --format '{{.Repository}}:{{.Tag}}' \
        | awk -F: 'NF==2 && $2 ~ /^rollback-[0-9]+$/ {print}' \
        | while read -r ref; do
            if ! docker ps -aq --filter "ancestor=$ref" | grep -q .; then
                docker rmi -f "$ref" >/dev/null 2>&1 || true
            fi
...
setup_deploy_service() {
    ...
    #!/bin/bash
    # Process /tmp/deploy/*.tar.xz -> verify, compare SHA, move to candidate, trigger build
    set -Eeuo pipefail
    IFS=$'\n\t'
    umask 077
    
    inbox_dir="/tmp/deploy"
    current_dir="/opt/frontend/deploy/current"
    candidate_dir="/opt/frontend/deploy/candidate"
    ...
    install -d -m 0700 "$current_dir" "$candidate_dir"
    
    exec 9>/run/frontend-deploy.lock
    if ! flock -n 9; then
    ...
    candidate="$(
        find "$inbox_dir" -maxdepth 1 -type f -name '*.tar.xz' \
            -printf '%T@ %p\n' 2>/dev/null \
          | sort -nr \
          | awk 'NR==1{print $2}'
    )"
    ...
    cur_sha=""
    if [[ -f "$current_dir/.current" ]]; then
        # expecting "rev sha256"
        read -r cur_rev cur_sha < "$current_dir/.current" || true
    fi
    
    if [[ -n "$cur_sha" && "$new_sha" == "$cur_sha" ]]; then
        log_i "already current (rev=${cur_rev:-unknown}, sha256 match); dropping $base"
        rm -f -- "$candidate"
        flock -u 9 || true
        exit 0
    fi
    ...
    if ! tar -tJf "$candidate" data/docker-compose.yaml data/.volumes data/.url >/dev/null 2>&1; then
        log_e "archive missing required entries: data/docker-compose.yaml, data/.volumes or data/.url"
        flock -u 9 || true
        exit 1
    fi
    ...
    b64_cfg="$(get_vol_b64 volume_frontend_nginx_config)"
    b64_tun="$(get_vol_b64 volume_frontend_tunnel)"
    
    [[ -n "$b64_cfg" ]] || die ".volumes missing volume_frontend_nginx_config"
    [[ -n "$b64_tun" ]] || die ".volumes missing volume_frontend_tunnel"
    
    check_b64_xz() {
        local name="$1" data="$2"
        local tmp
        tmp="$(mktemp -t "frontend.${name}.XXXXXXXX.xz")"
    ...
    check_b64_xz "volume_frontend_nginx_config" "$b64_cfg"
    check_b64_xz "volume_frontend_tunnel" "$b64_tun"
    
    archive_url="$(tar -xOJf "$candidate" data/.url 2>/dev/null | tr -d '\r' || true)"
    if [[ -z "$archive_url" ]]; then
        die "data/.url is empty or missing"
    fi
    log_i "archive_url in candidate: $archive_url"
    
    rm -f "$candidate_dir"/*.tar.xz 2>/dev/null || true
    
    mv -f -- "$candidate" "$candidate_dir/$base"
    chown root:root "$candidate_dir/$base" 2>/dev/null || true
    log_i "candidate updated: $base (rev=$rev sha256=$new_sha)"
...
setup_frontend() {
    install -d -m 0750 -o root -g root /opt/frontend/deploy/{current,candidate}
}
