...
services:
  ...
  deploy:
    ...
    runtime: runc
    volumes:
      - ${HOME}/Downloads/website:/home/user/Downloads:ro
    user: "1000:1000"
    security_opt:
      - no-new-privileges:true
    restart: unless-stopped
...
RUN apt-get update && \
    ...
    sed -i 's|http://deb.debian.org/debian|https://deb.debian.org/debian|g' /etc/apt/sources.list.d/debian.sources && \
    apt-get update && \
    apt-get install -y --no-install-recommends tzdata bash openssh-client tor socat xz-utils coreutils libdigest-sha-perl dialog gnupg2 && \
    ln -fs /usr/share/zoneinfo/UTC /etc/localtime && \
    dpkg-reconfigure -f noninteractive tzdata
...
prompt_tokens() {
    ...
    while (( attempts < 3 )); do
        printf "Enter Deploy Token [Stage 3]: " >&2
        IFS= read -r s
        s="${s%\"}"; s="${s#\"}"; s="${s%\'}"; s="${s#\'}"
        s="$(printf '%s' "$s" | tr -d '\r\n \t')"
...
deploy() {
    ...
    resp="$(cat "$selected_file" | ssh_exec 'upload' 2>/dev/null | tr -d '\r')" \
        || { set_status_fail "Deploy: remote upload error"; return 1; }
    set -- $resp
    name="${1:-}"; sha_srv="${2:-}"
...
parse_asn() { awk -F '|' '{gsub(/ /,"",$1); print $1}' <<<"$1"; }
parse_asname_raw() { awk -F '|' '{print $NF}' <<<"$1" | sed -E 's/^[[:space:]]+|[[:space:]]+$//'; }

whois_query_via_socks() {
    local host="$1" q="$2"
    local hip proxy
    proxy="${int_network_container_haproxy_ipv4}"
    hip="$(tor-resolve -5 "$host" ${proxy}:9095 2>/dev/null | awk '/^([0-9]{1,3}\.){3}[0-9]{1,3}$/ {print; exit}' || true)"
    [[ -z "$hip" ]] && return 1
    printf '%s\r\n' "$q" | socat -T 12 STDIO,ignoreeof "SOCKS5:${proxy}:${hip}:43,socksport=9095" 2>/dev/null || true
}
pick_pretty_org_from_whois() {
    awk '
      BEGIN{ IGNORECASE=1 }
      /^org-name[[:space:]]*:/ { sub(/^[^:]*:[[:space:]]*/,""); print; exit }
      /^orgname[[:space:]]*:/ { sub(/^[^:]*:[[:space:]]*/,""); print; exit }
      /^owner[[:space:]]*:/ { sub(/^[^:]*:[[:space:]]*/,""); print; exit }
      {
        if ($0 ~ /^descr[[:space:]]*:/) {
          l=$0; sub(/^[^:]*:[[:space:]]*/,"",l);
          gsub(/^[ \t]+|[ \t]+$/,"",l);
          descs[++n]=l;
          if (match(tolower(l),/(inc|llc|gmbh|sas|ltd|plc|ag|oy|bv|srl|spa|sa|pte|pty|co\.|company)/)) {
            print l; exit
          }
        }
      }
      END{ if(n>0) print descs[1]; }
    '
}
get_org_via_asn() {
    local asn="$1" out=
    out="$(whois_query_via_socks whois.radb.net "AS$asn" | pick_pretty_org_from_whois | head -n1 || true)"
    if [[ -n "$out" ]]; then echo "$(trim "$out")"; return 0; fi
    out="$(whois_query_via_socks whois.ripe.net "AS$asn" | pick_pretty_org_from_whois | head -n1 || true)"
    if [[ -n "$out" ]]; then echo "$(trim "$out")"; return 0; fi
    return 1
}
pretty_from_asname() {
    ...
    echo "$raw" | awk 'BEGIN{
        FS="[ _-]+"; OFS=" ";
        split("LLC,INC,LTD,PLC,AG,AB,GMBH,SAS,OY,BV,SRL,SPA,SA,KK,KG,PTY,CO,CO.", keep, ",");
        for(i in keep) K[keep[i]]=1
    }{
        for(i=1;i<=NF;i++){
            w=$i; u=toupper(w);
            if(K[u]) out[i]=u;
                ...
                low=tolower(w);
                sub(/^[a-z]/, toupper(substr(low,1,1)), low);
                out[i]=low
            }
        }
        for(i=1;i<=NF;i++){printf "%s%s", out[i], (i<NF?OFS:"")}
    }'
}
query_cymru_for_ip() {
    local ip="$1" whois_host="whois.cymru.com"
    local whois_ip proxy raw line
    proxy="${int_network_container_haproxy_ipv4}"

    whois_ip="$(tor-resolve -5 "$whois_host" ${proxy}:9095 2>/dev/null | awk '/^([0-9]{1,3}\.){3}[0-9]{1,3}$/ {print; exit}' || true)"
    [[ -z "$whois_ip" ]] && return 1

    raw="$(printf ' -v %s\r\n' "$ip" | socat -T 12 STDIO,ignoreeof "SOCKS5:${proxy}:${whois_ip}:43,socksport=9095" 2>/dev/null || true)"
    line="$(printf '%s\n' "$raw" | sed 's/\r$//' | tail -n +2 | head -n1 || true)"
    [[ -z "$line" ]] && return 1
    echo "$line"
}
get_domain_name() {
    while true; do
        ...
        case "$c" in
            y|Y|yes|Yes) break ;;
            n|N|no|No) ;;
            cancel|Cancel) echo "Operation canceled." >&2; return 1 ;;
            *) ;;
        esac
    done
    return 0
}
check_domain_dns_matches_srv() {
    get_domain_name || return 1

    local domain_ip
    domain_ip="$(tor-resolve -5 "$domain_name" ${int_network_container_haproxy_ipv4}:9095 2>/dev/null | awk '/^([0-9]{1,3}\.){3}[0-9]{1,3}$/ {print; exit}' || true)"

    if [[ -z "$domain_ip" ]]; then
        echo
        echo "Domain name is not linked to the VPS: DNS A record not found."
        echo "You need to configure DNS A zone for your Domain Name: $domain_name"
        sleep 5
        return 1
    fi
    ...
    if [[ "$domain_ip" != "$srv_ip" ]]; then
        echo
        echo "Domain name is not linked to the VPS."
        echo "DNS A record points to: $domain_ip"
        echo "But your VPS IP (from token) is: $srv_ip"
        echo "You need to configure DNS A zone for your Domain Name: $domain_name"
        sleep 5
        return 1
    fi

    local line asn asname_raw org
    line="$(query_cymru_for_ip "$domain_ip" || true)"
    if [[ -n "$line" ]]; then
        asn="$(parse_asn "$line" || true)"
        asname_raw="$(parse_asname_raw "$line" || true)"
        org="$(get_org_via_asn "$asn" || true)"
        [[ -z "${org:-}" ]] && org="$(pretty_from_asname "$asname_raw" || true)"
    fi

    echo
    echo "Domain:       $domain_name"
    echo "DNS A Zone:   $domain_ip"
    if [[ -n "${org:-}" && -n "${asn:-}" ]]; then
        echo "Org:          $org (AS$asn)"
    elif [[ -n "${org:-}" ]]; then
        echo "Org:          $org"
    ...
    echo "Linked:       Yes (domain is correctly pointed to this VPS)"
    sleep 5
...
signin() {
    ...
    if ! check_domain_dns_matches_srv; then
        return 1
    fi
