...
EOF
    ...
    # Fail-fast: ensure all build inputs exist before docker build
    for p in exit_a exit_b haproxy deploy; do
        [[ -f "${tmp_folder}/${rnd_proj_name}/${p}/Dockerfile" ]] || die "Missing Dockerfile: ${p}/Dockerfile"
    done
    [[ -f "${tmp_folder}/${rnd_proj_name}/docker-compose.yaml" ]] || die "Missing docker-compose.yaml"
    [[ -f "${tmp_folder}/${rnd_proj_name}/.env" ]] || die "Missing .env"
}
